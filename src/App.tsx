...
import { FiltersBar, SearchBar } from "./components/FilterBar";
...
export default function App() {
  ...
  function verifyExist(tasks: Todo[], task: string): boolean {
    for (const t of tasks) {
      if (t.task.toLowerCase() === task.toLowerCase()) return false;
    }
    return true;
  }
  ...
  function toggleStatus(task: string, checked: boolean) {
    setTodos((prev) => {
      return prev.map((t) => {
        if (t.task === task) return { ...t, status: checked ? "Done" : "Todo" };
        return t;
      });
    });
  }
